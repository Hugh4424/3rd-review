# 3rd-review V3 Protocol Freeze（Phase 0–1）
...
- `round=1` 的 runtime/previous receipts 必须是 `null`。后续轮次使用新的 `request_id`，并携带 `previous_receipts: {provider: receipt_hash}`；其 key 必须精确等于上一轮成功且有 native session 的 provider 集合。每个 provider 只续自己的 session，不允许跳轮、分叉、跨 provider 或自动 fresh。
...
私有 receipt 绑定 request/nonce/owner uid/runtime/provider/round/config snapshot/material hash/contract ref/session/profile/expiry，并保存 `parent_receipt_hash`。每轮 receipt 不可覆盖。broker 在解析 adapter 的最终文本后另存通用的 `result` payload；它没有业务 verdict，只让 wh-review 做合同校验。raw、diagnostic、result 只通过下面的固定接口读取：

```text
3rd-review read-private --runtime-id <id> --provider <provider> --nonce <nonce> --round <n> --ref raw|diagnostic|receipt|result
```

ref 不是文件名；除 `raw`、`diagnostic`、`receipt`、`result` 外一律 `REQUEST_INVALID`。Phase 1 写入 `$TMPDIR/3rd-review/<runtime>/<provider>/` 的 `0700/0600` 私有文件，并用 uid、nonce、lstat/realpath containment 拒绝 `..`、symlink、跨 runtime/provider 读取。

receipt 与 nonce 的终态 TTL 是 24 小时。active lease 不会被 TTL 或默认 wall-clock kill；过期终态返回 `NONCE_EXPIRED`，调用方只能创建新的 request。

### Phase 1 direct supervisor 的不可变约束

真实 provider 只能由一个 supervisor 以 `spawn(command, argv, { shell: false })` 启动：不经 shell、不能把 prompt 拼进命令字符串、不能共享 V2 runner。每次 attempt 的 stdout、stderr、receipt 仅写入 `$TMPDIR/3rd-review/<runtime>/<provider>/`，目录 `0700`、文件 `0600`；创建和读取都必须逐段 `lstat`，拒绝 symlink、`..` 与跨 runtime/provider 路径。

supervisor 没有默认 deadline，也不因 idle 自动重派或 fresh；它只记录 `last_activity_ms`、heartbeat、状态与退出信息。只有调用方显式给出 `deadline_seconds` 才终止整个 process group。用户取消、显式 deadline、输出字节上限、spawn/进程失败和非零退出必须分别留下终态 receipt；raw、stderr 或 receipt 未能落盘时改写为 `RUNTIME_UNAVAILABLE(persisted=false)`，不能报告成功。
