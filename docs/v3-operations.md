...
- recovery 在启动 provider 前就持久化“已消费”状态。若 broker 在该次 native continuation 中途崩溃，恢复槽仍然消耗；这是 at-most-once 安全语义，防止同一 native session 被静默重复执行。调用方会保留已有 supervisor receipt/diagnostic，并明确创建新 request 才能 fresh。
...
- `read-private` 必须带 nonce、provider 和明确 round，并只接受 `raw|diagnostic|receipt|result`；不能传任意文件路径。`result` 是 adapter 已提取、已脱敏的最终文本，不包含业务 verdict。

## 多轮业务审查

- 第 2 轮及以后必须是新的 request_id，并带上同 runtime/nonce、上一轮每个成功 session 的 `previous_receipts` 映射与新的 bounded delta。
- 每个映射项只续跑它自己的 native session；成功但没有 session 的 provider 保留为本轮结果，不进入下一轮；失败 provider 只保留诊断，不会 silent fresh。
- 每轮 receipt 和 final result 都不可覆盖，并以 `parent_receipt_hash` 串联。transport recovery 的一次性预算不限制正常的业务续跑轮数。
