...
export function cancellationSource(root, runtime_id, provider) {
  try { const raw = fs.readFileSync(path.join(runtimeDirectory(root, runtime_id), `.cancel-${provider}`), "utf8").trim(); try { const value = JSON.parse(raw); return value?.version === 1 && cancellationSources.has(value.source) ? value.source : INVALID_CANCELLATION_SOURCE; } catch { return cancellationSources.has(raw) ? raw : INVALID_CANCELLATION_SOURCE; } }
  catch { return null; }
}
