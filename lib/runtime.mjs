...
export function processIdentity(pid) {
  ...
  const result = spawnSync("ps", ["-o", "uid=", "-o", "lstart=", "-p", String(pid)], { encoding: "utf8", windowsHide: true });
  const match = String(result.stdout ?? "").trim().match(/^(\d+)\s+(.+)$/);
  return result.status === 0 && match ? { pid, uid: Number(match[1]), started: match[2] } : null;
}
