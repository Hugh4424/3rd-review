...
export function requestCancellation(root, runtime_id, provider, source = "caller") {
  if (typeof source !== "string" || !source.trim()) fail("REQUEST_INVALID", "cancel source must be a non-empty string");
  write(path.join(runtimeDirectory(root, runtime_id), `.cancel-${provider}`), { version: 1, source });
}
export function cancellationSource(root, runtime_id, provider) {
  try { const raw = fs.readFileSync(path.join(runtimeDirectory(root, runtime_id), `.cancel-${provider}`), "utf8").trim(); try { const value = JSON.parse(raw); return value?.version === 1 && typeof value.source === "string" ? value.source : "caller"; } catch { return raw || "caller"; } }
  catch { return null; }
}
...
export function claimProvider(root, runtime_id, provider, staleMs = 30_000) {
  const claims = path.join(runtimeDirectory(root, runtime_id), ".claims"); fs.mkdirSync(claims, { recursive: true, mode: 0o700 }); const directory = path.join(claims, provider); const reaper = path.join(claims, `.reap-${provider}`);
