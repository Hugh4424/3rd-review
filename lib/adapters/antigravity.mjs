...
const MAX_ARGV_PROMPT_BYTES = 64 * 1024;
...
function start(provider, cwd, prompt) {
  if (!provider.allow_host_state) fail("PROVIDER_HOST_STATE_UNACKNOWLEDGED", "Antigravity persists prompts and conversations in the native CLI profile; set providers.antigravity.allow_host_state=true only for trusted material");
  ...
  if (Buffer.byteLength(prompt, "utf8") > MAX_ARGV_PROMPT_BYTES) fail("PROMPT_TOO_LARGE", `Antigravity argv prompt exceeds ${MAX_ARGV_PROMPT_BYTES} bytes`);
  const argv = ["--new-project", "--mode", "plan", "--sandbox", "--dangerously-skip-permissions", "--print-timeout", "24h"];
