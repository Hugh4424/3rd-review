...
export default {
  ...
  start(provider, cwd, prompt) { const argv = ["exec", "-C", cwd, "-s", "read-only", "--skip-git-repo-check", "--ignore-user-config", "--ignore-rules"]; if (provider.model) argv.push("-m", provider.model); if (provider.effort) argv.push("-c", `model_reasoning_effort=${JSON.stringify(provider.effort)}`); argv.push("--json", "-"); return { ...plan(provider, cwd, argv, prompt), observeLine: jsonProgress }; },
  resume(provider, cwd, session, prompt) { const argv = ["exec", "resume", session, "--skip-git-repo-check", "--ignore-user-config", "--ignore-rules"]; if (provider.model) argv.push("-m", provider.model); if (provider.effort) argv.push("-c", `model_reasoning_effort=${JSON.stringify(provider.effort)}`); argv.push("--json", "-"); return { ...plan(provider, cwd, argv, prompt), observeLine: jsonProgress }; },
  parse,
};
