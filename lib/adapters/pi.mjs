...
const reviewSystemPrompt = "You are an independent, read-only review analyst. Follow the supplied review instruction exactly. Use only the approved review tool and return only the requested review.";
const reviewInstruction = "Review only the supplied instruction and the frozen packet files. Use read only with logical bundle/<file> paths. Do not access parent directories, use git, shell, network, host paths, absolute paths, or file URIs. Findings may name only change_id, context anchors, or logical bundle/<file> names. Do not use write or edit tools. Return only the requested review.";
