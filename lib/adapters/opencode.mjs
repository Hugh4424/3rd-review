...
const reviewInstruction = "Use the Read tool to open review-input.md in the isolated working directory. Read it in chunks with offsets until EOF (end of file), including all attachment head and tail markers. Then return only the requested review.";
...
export default {
  ...
  start(provider, cwd, prompt, runtime) { const config = profile(runtime, provider); writeFile(path.join(cwd, "review-input.md"), prompt); const argv = ["run", "--pure", "--dir", cwd, "--format", "json"]; if (provider.model) argv.push("--model", provider.model); if (provider.effort) argv.push("--variant", provider.effort); argv.push("--agent", "third-review-readonly", reviewInstruction); return { ...plan(provider, cwd, argv, null, { OPENCODE_CONFIG: config, OPENCODE_DISABLE_CLAUDE_CODE: "1" }), observeLine: jsonProgress }; },
  resume(provider, cwd, session, prompt, runtime) { const config = profile(runtime, provider); writeFile(path.join(cwd, "review-input.md"), prompt); const argv = ["run", "--session", session, "--pure", "--dir", cwd, "--format", "json"]; if (provider.model) argv.push("--model", provider.model); if (provider.effort) argv.push("--variant", provider.effort); argv.push("--agent", "third-review-readonly", reviewInstruction); return plan(provider, cwd, argv, null, { OPENCODE_CONFIG: config, OPENCODE_DISABLE_CLAUDE_CODE: "1" }); }, parse,
};
