...
import { invalid, jsonProgress, lines, nonempty, plan, writeFile } from "./shared.mjs";
...
function profile(cwd) { const root = path.join(cwd, ".broker-profile"); fs.mkdirSync(root, { recursive: true, mode: 0o700 }); const file = path.join(root, "opencode.json"); writeFile(file, `${JSON.stringify({ "$schema": "https://opencode.ai/config.json", agent: { "third-review-readonly": { description: "Read-only review", mode: "primary", permission: { "*": "deny", read: "allow", glob: "allow", grep: "allow", list: "allow" } } } })}\n`); return file; }
...
export default {
  ...
  start(provider, cwd, prompt) { const config = profile(cwd); const argv = ["run", "--pure", "--dir", cwd, "--format", "json"]; if (provider.model) argv.push("--model", provider.model); if (provider.effort) argv.push("--variant", provider.effort); argv.push("--agent", "third-review-readonly", reviewInstruction); return { ...plan(provider, cwd, argv, prompt, { OPENCODE_CONFIG: config, OPENCODE_DISABLE_CLAUDE_CODE: "1" }), observeLine: jsonProgress }; },
  resume(provider, cwd, session, prompt) { const config = profile(cwd); const argv = ["run", "--session", session, "--pure", "--dir", cwd, "--format", "json"]; if (provider.model) argv.push("--model", provider.model); if (provider.effort) argv.push("--variant", provider.effort); argv.push("--agent", "third-review-readonly", reviewInstruction); return plan(provider, cwd, argv, prompt, { OPENCODE_CONFIG: config, OPENCODE_DISABLE_CLAUDE_CODE: "1" }); }, parse,
};
