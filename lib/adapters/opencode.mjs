...
const reviewInstruction = "Review only the supplied instruction and the frozen files under bundle/ in the current isolated attachment workspace. Do not use git, shell, network, host paths, or any files outside this workspace. Return exactly the requested JSON object: no preface, explanation, or Markdown code fence before or after the JSON.";
const controller = fileURLToPath(new URL("./opencode-supervised-cli.mjs", import.meta.url));

function cursorFor(messages) {
  const last = messages.at(-1); if (!last) return null;
  return `${last.info?.id ?? ""}:${last.parts?.at(-1)?.id ?? ""}:${createHash("sha256").update(JSON.stringify(last)).digest("hex")}`;
}
...
export function createOpenCodeProbe({ url, fetchImpl = fetch } = {}) {
  return async ({ session_id, cursor = null, signal } = {}) => {
    ...
    try {
      ...
      const nextCursor = cursorFor(messages); const terminal = assistantTerminal(messages);
...
function executionPlan(provider, cwd, prompt, session = null) {
  ...
  const specification = Buffer.from(JSON.stringify({ command: provider.command, url, port, clientArgv }), "utf8").toString("base64url");
