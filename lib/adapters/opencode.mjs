...
export default {
  ...
  start(provider, cwd, prompt) { const argv = ["run", "--pure", "--dir", cwd, "--format", "json"]; if (provider.model) argv.push("--model", provider.model); if (provider.effort) argv.push("--variant", provider.effort); return { ...plan(provider, cwd, argv, prompt, { OPENCODE_DISABLE_CLAUDE_CODE: "1" }), observeLine: jsonProgress }; },
  resume(provider, cwd, session, prompt) { const argv = ["run", "--session", session, "--pure", "--dir", cwd, "--format", "json"]; if (provider.model) argv.push("--model", provider.model); if (provider.effort) argv.push("--variant", provider.effort); return plan(provider, cwd, argv, prompt, { OPENCODE_DISABLE_CLAUDE_CODE: "1" }); }, parse,
};
