...
try {
  ...
  let sessionID = specification.session ?? null; let lineBuffer = "";
  ...
  if (code === 0 && sessionID) {
    ...
    else { process.stderr.write(`OpenCode session ${sessionID} was not terminal after client exit (${terminal.status})\n`); exitCode = 1; }
  }
  if (code === 0 && !sessionID) { process.stderr.write("OpenCode client exited without a session id\n"); exitCode = 1; }
