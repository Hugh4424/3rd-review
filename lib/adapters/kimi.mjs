...
function wireText(item) {
  ...
  if (params?.type === "TextPart" || (params?.type === "ContentPart" && params.payload?.type === "text")) return nonempty(params.payload?.text);
  return null;
}
...
function wirePlan(provider, cwd, prompt, session = null) {
  ...
  const argv = ["--wire", "--work-dir", cwd, "--skills-dir", path.join(cwd, "skills"), "--afk"];
