...
export default {
  ...
  start(provider, cwd, prompt, runtime) { const p = profile(runtime, provider); const argv = ["--print", "--input-format", "text", "--output-format", "stream-json", "--work-dir", cwd, "--agent-file", p.file, "--skills-dir", p.skills]; if (provider.model) argv.push("--model", provider.model); if (provider.thinking === true) argv.push("--thinking"); if (provider.thinking === false) argv.push("--no-thinking"); return { ...plan(provider, cwd, argv, prompt), observeLine }; },
