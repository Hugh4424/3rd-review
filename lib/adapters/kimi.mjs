...
function profile(runtime, provider, cwd) {
  ...
  const frozen = path.join(runtime, "workspace", provider.id); const frozenSkills = path.join(frozen, "skills");
  return { file: path.join(root, "reviewer.yaml"), workDir: fs.existsSync(frozen) ? frozen : cwd, skills: fs.existsSync(frozenSkills) ? frozenSkills : path.join(cwd, "skills") };
}
