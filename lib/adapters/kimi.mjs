...
function profile(runtime, provider, cwd) {
  const root = restrictedFiles(runtime, provider); const bundle = path.join(cwd, "bundle"); const attached = fs.existsSync(bundle) ? " The complete frozen review bundle is in the relative `bundle` directory of your current workspace. Use the provided ReadFile tool with relative bundle paths (for example `bundle/review-packet.v1.json`) to read review-packet.v1.json, contracts, and skills through to the end; this is permitted, and do not seek or use a shell." : ""; const prompt = `You are a read-only independent reviewer. Do not modify files, execute commands, access the network, or delegate. Review only the supplied prompt and attached workspace. Read and apply attached skills when relevant, but never use skills outside this private workspace.${attached}\n`;
