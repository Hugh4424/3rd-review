...
function profile(runtime, provider, cwd) {
  const root = restrictedFiles(runtime, provider); const bundle = path.join(cwd, "bundle"); const attached = fs.existsSync(bundle) ? ` The complete frozen review bundle is at ${bundle}. Read review-packet.v1.json, contracts, and skills from that bundle through to the end.` : ""; const prompt = `You are a read-only independent reviewer. Do not modify files, execute commands, access the network, or delegate. Review only the supplied prompt and attached workspace. Read and apply attached skills when relevant, but never use skills outside this private workspace.${attached}\n`;
