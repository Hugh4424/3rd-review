...
export default {
  ...
  // The trusted file_only wrapper enforces read-only isolation. Do not create
  // an agent profile beside the bundle: that would make host configuration a
  // provider-visible attachment. Kimi receives only its isolated cwd/skills.
  start(provider, cwd, prompt) { const argv = ["--print", "--input-format", "text", "--output-format", "stream-json", "--work-dir", cwd, "--skills-dir", path.join(cwd, "skills")]; if (provider.model) argv.push("--model", provider.model); if (provider.thinking === true) argv.push("--thinking"); if (provider.thinking === false) argv.push("--no-thinking"); return { ...plan(provider, cwd, argv, `${reviewInstruction}\n\n${prompt}`), observeLine }; },
