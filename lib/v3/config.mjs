...
const DEFAULT_FIELDS = new Set(["deadline_seconds", "max_input_bytes", "max_output_bytes"]);
...
export function validateConfig(input) {
  ...
  for (const [id, value] of Object.entries(providersInput)) {
    ...
    if (providers[id].auth_mode === "env" && providers[id].auth_env.length === 0) {
      throw new ProtocolError("CONFIG_INVALID", `providers.${id}.auth_mode=env requires auth_env names`);
    }
    if (providers[id].auth_mode === "config_ref" && providers[id].config_ref === null) {
      throw new ProtocolError("CONFIG_INVALID", `providers.${id}.auth_mode=config_ref requires config_ref`);
    }
  }
...
export function loadConfig(file = defaultConfigPath()) {
  ...
  try {
    ...
    if (!parentStat.isDirectory() || parentStat.isSymbolicLink() || (parentStat.mode & 0o077) !== 0) {
      throw new ProtocolError("CONFIG_INVALID", "config directory must be a private non-symlink directory (for example 0700)");
    }
  } catch (error) {
    if (error instanceof ProtocolError) throw error;
    throw new ProtocolError("CONFIG_INVALID", `config directory is not readable: ${error.message}`);
  }
