  ...
  for (let tierIndex = start; tierIndex <= end; tierIndex += 1) {
    ...
    const pending = selected.map(async (id) => {
      ...
      try {
        ...
        return { id, status: transportEligible ? "completed" : "failed", execution_eligible: eligible, session_id: eligible && typeof result?.session_id === "string" ? result.session_id : null, error_code: eligible ? null : (transportEligible ? "HOST_UNKNOWN" : (typeof result?.error_code === "string" ? result.error_code : "PROVIDER_PROTOCOL_INCOMPLETE")), persisted: result?.persisted !== false };
      } catch (error) {
