...
function privateFile(file) {
  ...
  try { fd = fs.openSync(file, "wx", 0o600); } catch (error) {
    throw new ProtocolError(error.code === "EEXIST" || error.code === "ELOOP" ? "BINDING_MISMATCH" : "RUNTIME_UNAVAILABLE", "private output path already exists or is unsafe", { cause: error.code ?? error.message });
  }
...
export function cancelPersistedAttempt({ active_path, attempt_id }) {
  ...
  if (typeof state.pid_fingerprint !== "string" || state.pid_fingerprint.length === 0) throw new ProtocolError("BINDING_MISMATCH", "active process has no verifiable identity; refusing to signal it");
  if (state.pid_fingerprint !== processFingerprint(state.pid)) throw new ProtocolError("PROCESS_DIED", "active process identity changed before cancellation");
