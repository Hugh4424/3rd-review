...
function interruptedResult(job, request) {
  const providers = Object.entries(job.rounds?.[request.round]?.providers ?? {}).map(([provider, state]) => (
    job.providers?.[provider]?.rounds?.[request.round]?.result ?? {
      id: provider, status: state.status ?? "failed", execution_eligible: false, session_id: null,
      runtime_id: job.runtime_id, error_code: "CONTINUATION_INTERRUPTED", persisted: true,
    }
  ));
  return {
    protocol_version: 3, request_id: request.request_id, nonce: request.nonce, runtime_id: job.runtime_id,
    config_hash: job.config_hash, selected_tier: null, stop_reason: "continuation_interrupted", providers,
  };
}
...
export class JobStore {
  ...
  begin({ request, config_hash, config_snapshot }) {
    ...
    const job = {
      ...
      contract_ref: request.contract_ref, host_hint: request.host_hint, created_at_ms: now, expires_at_ms: now + RUNTIME_TTL_MS,
  ...
  beginContinuation({ request, config_hash }) {
    ...
    if (JSON.stringify(request.host_hint) !== JSON.stringify(job.host_hint)) throw new ProtocolError("BINDING_MISMATCH", "continuation host identity changed");
    ...
    if (indexed !== null) {
      ...
      const job = this.readJob(indexed.runtime_id);
      if (job.active_round === request.round) {
        const lock = path.join(this.runtimePath(request.runtime_id), ROUND_LOCK);
        const held = privateJson(lock, null);
        if (held && !processAlive(held.pid)) {
          const { active_round, ...stable } = job;
          const terminal = { ...stable, status: "interrupted", result: interruptedResult(job, request) };
          writeJson(this.jobPath(request.runtime_id), terminal);
          try { fs.unlinkSync(lock); } catch { /* stale lock is no longer authoritative */ }
          return { existing: true, job: terminal };
        }
      }
    ...
    if (!reserveRoundLock(roundLock, request)) {
      ...
      if (held && !processAlive(held.pid)) {
        ...
        fs.unlinkSync(indexFile);
        throw new ProtocolError("CONTINUATION_INTERRUPTED", "a prior continuation crashed; do not silently rerun its native sessions");
      }
  ...
  commitProvider({ runtime_id, provider, request, config_hash, config_snapshot, profile_hash, result }) {
    ...
    const receipt = {
      ...
      contract_ref: request.contract_ref, host_hint: job.host_hint, session_id: result.session_id ?? null,
  ...
  gcExpired() {
    ...
    for (const entry of fs.readdirSync(this.indexRoot, { withFileTypes: true })) {
      ...
      if (this.now() <= job.expires_at_ms) continue;
      const live = this.listActive(job.runtime_id).filter((attempt) => attempt.terminal !== true && processAlive(attempt.pid));
