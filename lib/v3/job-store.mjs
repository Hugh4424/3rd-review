...
function reserveIndex(file) {
  ...
  try {
    ...
    fs.writeFileSync(fd, '{"creating":true}\n', "utf8");
...
export class JobStore {
  ...
  begin({ request, config_hash, config_snapshot }) {
    ...
    if (indexed) {
      if (indexed.creating === true) throw new ProtocolError("DUPLICATE_ACTIVE_REQUEST", "request creation is already active");
  ...
  commitProvider({ runtime_id, provider, request, config_hash, config_snapshot, profile_hash, result }) {
    ...
    const updated = { ...job, expires_at_ms: this.now() + RUNTIME_TTL_MS, providers: { ...job.providers, [provider]: { receipt_hash: digest, result: publicResult } } };
  ...
  readPrivate({ runtime_id, provider, nonce, ref: requested }) {
    ...
    if (!job.providers[provider]) throw new ProtocolError("BINDING_MISMATCH", "provider has no committed private receipt for this job");
  ...
  gcExpired() {
    ...
    for (const entry of fs.readdirSync(this.indexRoot, { withFileTypes: true })) {
      ...
      let job;
      try { job = this.readJob(indexed.runtime_id); } catch (error) { if (error.code === "BINDING_MISMATCH") continue; throw error; }
