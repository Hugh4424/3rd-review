...
export class LiveBroker {
  ...
  async #execute(provider, request, runtimeId, config, options, store) {
    ...
    return this.#commitProvider(store, { runtimeId, provider, request, config, result });
  }
  ...
  async #runContinuation({ request, config, options, store }) {
    ...
    const providers = await Promise.all(Object.keys(request.previous_receipts).sort().map(async (providerId) => {
      ...
      try {
        ...
        return this.#commitProvider(store, { runtimeId, provider, request, config, result: { execution_eligible: false, session_id: null, error_code: error instanceof ProtocolError ? error.code : "ADAPTER_FAILED", persisted: true, metrics: { elapsed_ms: 0, input_bytes: request.material.bytes, output_bytes: 0 } } });
      }
  ...
  #commitProvider(store, { runtimeId, provider, request, config, result }) {
    try {
