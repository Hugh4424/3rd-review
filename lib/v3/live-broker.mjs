...
export class LiveBroker {
  ...
  async #runPlan(adapter, plan, providerId, runtimeId, config, inputBytes) {
    ...
    if (attempt.status !== "completed" || attempt.persisted !== true || !attempt.stdout_path) {
      try { redactFile(attempt.stdout_path, plan.redact_values); redactFile(attempt.stderr_path, plan.redact_values); } catch { /* receipt remains the source of a storage failure */ }
      return {
    ...
    let parsed;
    try {
      redactFile(attempt.stdout_path, plan.redact_values);
      redactFile(attempt.stderr_path, plan.redact_values);
