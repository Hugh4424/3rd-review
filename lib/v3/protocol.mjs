...
export function validateRequest(request, { maxInputBytes = 524_288 } = {}) {
  ...
  if (request.round === 1) {
    ...
    if (typeof request.previous_receipt_hash !== "string" || !SHA256.test(request.previous_receipt_hash)) {
      throw invalid("round 2+ previous_receipt_hash must be sha256:<hex>");
    }
  }
...
export function assertPrivateRef(value) {
  if (value !== "raw" && value !== "diagnostic" && value !== "receipt") throw invalid("ref must be raw, diagnostic, or receipt");
  return value;
}
