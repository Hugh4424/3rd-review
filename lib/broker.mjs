...
import { requireFileOnlySandbox } from "./attachment-sandbox.mjs";
import { planDelivery, prepareAttachments, prepareWritableAttachmentView, probeAttachmentWorkspace, validateAttachmentRoot, validateAttachments, validateContinuationTriad, verifyFrozenAttachments } from "./attachments.mjs";
...
export class Broker {
  ...
  async doctor({ attachmentRoot = null } = {}) {
    ...
    const output = await Promise.all(Object.values(this.config.providers).map(async (provider) => {
      const baseCapabilities = adapter(provider.id).capabilities;
      const probeFailed = attachmentProbe.get(provider.id) === false;
      const capabilities = probeFailed ? { ...baseCapabilities, attachment_delivery: [] } : baseCapabilities;
      ...
      if (probeFailed) return { provider: provider.id, status: "unavailable", capabilities, error: { code: "ATTACHMENT_PROBE_FAILED", message: "attachment workspace probe failed" } };
  ...
  async run(raw) {
    ...
    if (continuing) {
      ...
      if (state.attachments && input.attachments) {
        if (input.attachments.delivery !== "file_only" || input.attachments.delivery !== state.attachments.requested_delivery) fail("MATERIAL_INCOMPLETE", "continuation attachment delivery must match the initial file_only delivery");
        ...
        state = updateRuntime(this.config.runtime.root, state.runtime_id, (next) => ({ ...next, continuation_materials: [...(next.continuation_materials ?? []), { sequence: binding.sequence, manifest_hash: checked.manifest_hash, delivery_manifest_hash: binding.delivery_manifest_hash, initial_material_manifest_hash: next.attachments.manifest_hash, provider_sessions: {} }] }));
      }
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider) {
    ...
    const runtime = runtimeDirectory(this.config.runtime.root, runtime_id); const worker = adapter(entry.id); let cwd; let providerPrompt = prompt; let deliveryUsed = prior?.delivery_used ?? null; let delivery = prior?.delivery ?? null; let attachmentKey = entry.id; let attachmentStored = null;
    try {
      if (input.attachments) {
        ...
        delivery = { delivery_mode: deliveryUsed, material_manifest_hash: checked.manifest_hash, total_bytes: checked.total_bytes, provider_visible_attachment_manifest: checked.files.map(({ target: destination, sha256, size }) => ({ destination, sha256, size })) };
        const planned = planDelivery(worker, checked, prompt, this.config.runtime.max_prompt_bytes);
        deliveryUsed = planned.delivery_mode;
        delivery = { delivery_mode: planned.delivery_mode, material_manifest_hash: planned.material_manifest_hash, total_bytes: planned.total_bytes, provider_visible_attachment_manifest: planned.provider_visible_attachment_manifest };
        if (deliveryUsed === "file_only") requireFileOnlySandbox();
    ...
    } catch (error) { return this.#setupFailure(runtime_id, entry, deliveryUsed, delivery, error, entry.continuation && prior?.status === "completed"); }
    let plan;
    try { if (!worker.promptViaStdin) fs.writeFileSync(path.join(cwd, "review-input.md"), providerPrompt, { mode: 0o600 }); plan = entry.continuation ? worker.resume(provider, cwd, prior.session_id, providerPrompt, runtime) : worker.start(provider, cwd, providerPrompt, runtime); }
    catch (error) { return this.#setupFailure(runtime_id, entry, deliveryUsed, delivery, error, entry.continuation && prior?.status === "completed"); }
    ...
    if (cancelled) { const source = cancellationSource(this.config.runtime.root, runtime_id, entry.id); const invalidSource = source === INVALID_CANCELLATION_SOURCE; return this.#store(runtime_id, entry, { status: "cancelled", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, cancellation_source: source, error: invalidSource ? { code: "CANCEL_SOURCE_INVALID", message: "cancel marker source is invalid", source } : { code: "CANCELLED", message: `cancel was requested by ${source}`, source } }); }
    if (!result.ok) return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, error: result.error, diagnostic: short(`${result.stdout}\n${result.stderr}`) });
    const parsed = worker.parse(result.stdout, result.stderr);
    if (!parsed.ok) return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, error: parsed.error, diagnostic: short(`${result.stdout}\n${result.stderr}`) });
    if (entry.continuation && input.attachments && parsed.session_id !== prior.session_id) return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, error: { code: "MATERIAL_INCOMPLETE", message: "continuation provider did not preserve its native session" } });
    if (entry.continuation && input.attachments) updateRuntime(this.config.runtime.root, runtime_id, (next) => ({ ...next, continuation_materials: (next.continuation_materials ?? []).map((item) => item.sequence === next.round ? { ...item, provider_sessions: { ...item.provider_sessions, [entry.id]: prior.session_id } } : item) }));
  ...
  #setupFailure(runtime_id, entry, deliveryUsed, delivery, error, preservePrior = false) {
