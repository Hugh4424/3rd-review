...
const WORKFLOWHUB_RESULT_PROTOCOL = "workflowhub-result.v1";
...
function workflowHubResult(value, checked) {
  return {
    provider: value.provider ?? null,
    status: value.status === "skipped" ? "failed" : value.status,
    result_protocol: WORKFLOWHUB_RESULT_PROTOCOL,
...
function request(value, config) {
  ...
  if (value.required_result_protocol !== undefined && value.required_result_protocol !== WORKFLOWHUB_RESULT_PROTOCOL) fail("PROTOCOL_INCOMPATIBLE", "required result protocol is not supported");
  if (value.required_result_protocol === WORKFLOWHUB_RESULT_PROTOCOL && !value.attachments) fail("REQUEST_INVALID", "workflowhub-result.v1 requires complete attachments on every round");
  ...
  if (value.provider_allowlist !== undefined) {
    if (!Array.isArray(value.provider_allowlist) || value.provider_allowlist.length === 0 || value.provider_allowlist.some((provider) => !config.providers[provider] || sameSource(provider, value.host_provider, config)) || new Set(value.provider_allowlist).size !== value.provider_allowlist.length) fail("REQUEST_INVALID", "provider_allowlist must contain unique configured heterologous providers");
  }
...
export class Broker {
  ...
  async run(raw) {
    const input = request(raw, this.config); const promptBytes = Buffer.byteLength(input.prompt, "utf8"); if (promptBytes > this.config.runtime.max_prompt_bytes && input.attachments?.delivery !== "always_embed") fail("PROMPT_TOO_LARGE", `prompt exceeds ${this.config.runtime.max_prompt_bytes} bytes`);
    ...
    if (initialChecked && input.required_result_protocol !== WORKFLOWHUB_RESULT_PROTOCOL) validateFileOnlyTriad(initialChecked);
    ...
    if (continuing) {
      ...
      if (state.attachments && input.attachments) {
        ...
        const binding = input.required_result_protocol === WORKFLOWHUB_RESULT_PROTOCOL
    ...
    const output = []; if (continuing && entries.length === 0) { const unavailableProvider = input.required_result_protocol === WORKFLOWHUB_RESULT_PROTOCOL && input.provider_allowlist?.length === 1 ? input.provider_allowlist[0] : null; const result = { provider: unavailableProvider, status: "failed", error: { code: "NO_CONTINUABLE_SESSION", message: "no successful provider session is available" } }; return this.#finish(state.runtime_id, input, [input.required_result_protocol === WORKFLOWHUB_RESULT_PROTOCOL ? workflowHubResult(result, deliveryChecked) : result], null); }
  ...
  async #runProvider(runtime_id, input, entry, continuationMaterial, deliveryChecked) {
    const project = (value) => input.required_result_protocol === WORKFLOWHUB_RESULT_PROTOCOL ? workflowHubResult(value, deliveryChecked) : value;
    ...
    if (entry.skip) return project({ provider: entry.id, tier: entry.tier, status: "skipped", error: { code: entry.skip, message: "host provider cannot review itself" } });
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial, deliveryChecked) {
    ...
    try {
      ...
      if (input.attachments) {
        ...
        if (deliveryUsed === "file_only" && input.required_result_protocol !== WORKFLOWHUB_RESULT_PROTOCOL) validateFileOnlyTriad(checked);
        const planned = planDelivery(worker, checked, prompt, this.config.runtime.max_prompt_bytes, { requireTriad: input.required_result_protocol !== WORKFLOWHUB_RESULT_PROTOCOL });
    ...
    const result = await execute(plan, {
      ...
      validateCompleted: (raw) => worker.parse(raw.stdout, raw.stderr, plan.expectedSession).ok,
    ...
    const rawRefs = this.#storeRaw(runtime, provider.runtime_key, result.stdout, result.stderr);
    ...
    const parsed = worker.parse(result.stdout, result.stderr, plan.expectedSession);
