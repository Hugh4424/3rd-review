...
export class Broker {
  ...
  async run(raw) {
    ...
    let state = continuing ? updateRuntime(this.config.runtime.root, continuing, (next) => ({ ...next, owner: { pid: process.pid, started_at_ms: Date.now() } })) : createRuntime(this.config.runtime.root, this.config.runtime.ttl_hours, input.host_provider, this.config.runtime.orphan_timeout_ms);
