...
import { appendEmbedded, prepareAttachments, prepareWritableAttachmentView, probeAttachmentWorkspace, validateAttachmentRoot, validateAttachments, verifyFrozenAttachments } from "./attachments.mjs";
...
export class Broker {
  ...
  async run(raw) {
    ...
    const selected = continuing ? Object.keys(state.providers).filter((id) => id !== input.host_provider && (!allowlist || allowlist.has(id)) && state.providers[id].status === "completed" && typeof state.providers[id].session_id === "string" && state.providers[id].session_id.length > 0) : null;
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider) {
    ...
    const runtime = runtimeDirectory(this.config.runtime.root, runtime_id); const worker = adapter(entry.id); let cwd; let providerPrompt = prompt; let deliveryUsed = prior?.delivery_used ?? null;
    try {
      if (input.attachments) {
        ...
        deliveryUsed = negotiateDelivery(worker, input.attachments.delivery, prepared.embedded_text);
        ...
        const frozen = verifyFrozenAttachments(runtime, entry.id, state.attachments); deliveryUsed = prior?.delivery_used;
        ...
        if (deliveryUsed === "always_embed") { if (frozen.embedded_text === null) fail("ATTACHMENT_IMMUTABLE", "frozen attachments are not embeddable"); cwd = path.join(runtime, "embed", entry.id); if (!fs.existsSync(cwd)) fail("ATTACHMENT_IMMUTABLE", "embedded provider workspace is unavailable"); }
        else cwd = frozen.cwd;
      } else { cwd = path.join(runtime, "workspace", entry.id); fs.mkdirSync(path.join(cwd, "skills"), { recursive: true, mode: 0o700 }); }
      if (worker.requiresWritableCwd && state.attachments) cwd = prepareWritableAttachmentView(runtime, entry.id, state.attachments).cwd;
    } catch (error) { return this.#setupFailure(runtime_id, entry, deliveryUsed, error, entry.continuation && prior?.status === "completed"); }
    let plan;
    ...
    const deliveryOutcome = deliveryUsed ? { delivery_used: deliveryUsed } : {};
  ...
  #setupFailure(runtime_id, entry, deliveryUsed, error, preservePrior = false) {
    const result = { status: "failed", ...(deliveryUsed ? { delivery_used: deliveryUsed } : {}), error: publicError(error) };
