...
import { appendEmbedded, prepareAttachments, prepareWritableAttachmentView, validateAttachmentRoot, validateAttachments, verifyFrozenAttachments } from "./attachments.mjs";
...
export class Broker {
  ...
  async doctor({ attachmentRoot = null } = {}) {
    const attachmentRootStatus = this.#attachmentRootStatus(attachmentRoot);
    ...
    return { version: 4, capabilities: { attachments: attachmentRootStatus.status !== "unavailable", cancel_source: true }, attachment_root: attachmentRootStatus, verification: "executable_only", note: "doctor does not verify model authentication or a real review", providers: output };
  }
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider) {
    ...
    if (cancelled) { const source = cancellationSource(this.config.runtime.root, runtime_id, entry.id); return this.#store(runtime_id, entry, { status: "cancelled", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, cancellation_source: source, error: { code: "CANCELLED", message: `cancel was requested by ${source}`, source } }); }
