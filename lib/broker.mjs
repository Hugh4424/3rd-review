...
import { planDelivery, prepareAttachments, prepareWritableAttachmentView, probeAttachmentWorkspace, validateAttachmentRoot, validateAttachments, verifyFrozenAttachments } from "./attachments.mjs";
...
export class Broker {
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider) {
    ...
    const runtime = runtimeDirectory(this.config.runtime.root, runtime_id); const worker = adapter(entry.id); let cwd; let providerPrompt = prompt; let deliveryUsed = prior?.delivery_used ?? null; let delivery = prior?.delivery ?? null;
    try {
