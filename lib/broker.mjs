...
import { cancellationRequested, cancellationSource, createRuntime, cleanup, isAlive, readRuntime, requestCancellation, runtimeDirectory, terminateProcessTree, updateRunningProvider, updateRuntime } from "./runtime.mjs";
...
export class Broker {
  ...
  cancel(runtime_id, provider) {
    ...
    requestCancellation(this.config.runtime.root, runtime_id, provider, "caller");
  ...
  async run(raw) {
    ...
    const state = continuing ? updateRuntime(this.config.runtime.root, continuing, (next) => ({ ...next, owner: { pid: process.pid, started_at_ms: Date.now() } })) : createRuntime(this.config.runtime.root, this.config.runtime.ttl_hours, input.host_provider, this.config.runtime.orphan_timeout_ms);
  ...
  async #runProvider(runtime_id, prompt, entry) {
    if (this.shuttingDown) return { provider: entry.id, tier: entry.tier, status: "cancelled", cancellation_source: "broker_shutdown", error: { code: "CANCELLED", message: "broker is shutting down" } };
    ...
    const runtime = runtimeDirectory(this.config.runtime.root, runtime_id); const cwd = path.join(runtime, "workspace", entry.id); fs.mkdirSync(cwd, { recursive: true, mode: 0o700 }); fs.writeFileSync(path.join(cwd, "review-input.md"), prompt, { mode: 0o600 });
    ...
    if (cancelled) { const source = cancellationSource(this.config.runtime.root, runtime_id, entry.id); return this.#store(runtime_id, entry, { status: "cancelled", duration_ms: result.duration_ms, ...telemetry, cancellation_source: source, error: { code: "CANCELLED", message: `cancel was requested by ${source}` } }); }
    if (!result.ok) return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, error: result.error, diagnostic: short(`${result.stdout}\n${result.stderr}`) });
    const parsed = worker.parse(result.stdout, result.stderr);
    if (!parsed.ok) return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, error: parsed.error, diagnostic: short(`${result.stdout}\n${result.stderr}`) });
    return this.#store(runtime_id, entry, { status: "completed", duration_ms: result.duration_ms, ...telemetry, session_id: parsed.session_id, usage: parsed.usage, output: parsed.text });
  }
