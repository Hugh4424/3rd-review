...
import { cancellationRequested, createRuntime, cleanup, isAlive, readRuntime, requestCancellation, runtimeDirectory, terminateProcess, updateRuntime } from "./runtime.mjs";
...
export class Broker {
  ...
  async doctor() {
    const output = await Promise.all(Object.values(this.config.providers).map(async (provider) => {
      ...
      try { const result = await execute(adapter(provider.id).doctor(provider, work), { maxOutputBytes: 65536 }); return result.ok ? { provider: provider.id, status: "ready" } : { provider: provider.id, status: "unavailable", error: result.error }; }
      finally { fs.rmSync(work, { recursive: true, force: true }); }
    }));
    return { version: 4, providers: output };
  }
  ...
  async run(raw) {
    ...
    const output = []; if (continuing && entries.length === 0) return this.#finish(state.runtime_id, input, [{ provider: null, status: "failed", error: { code: "NO_CONTINUABLE_SESSION", message: "no successful provider session is available" } }]);
    if (continuing) output.push(...await Promise.all(entries.map((entry) => this.#runProvider(state.runtime_id, input.prompt, entry))));
    else {
      for (const tier of entries) { const results = await Promise.all(tier.map((entry) => this.#runProvider(state.runtime_id, input.prompt, entry))); output.push(...results); if (results.some((result) => result.status === "completed")) break; }
    }
    return this.#finish(state.runtime_id, input, output);
  }
  ...
  async #runProvider(runtime_id, prompt, entry) {
    ...
    let lastHeartbeat = 0;
    const heartbeat = () => { const current = Date.now(); if (current - lastHeartbeat < 1_000) return; lastHeartbeat = current; updateRuntime(this.config.runtime.root, runtime_id, (next) => next.providers[entry.id]?.status === "running" ? ({ ...next, providers: { ...next.providers, [entry.id]: { ...next.providers[entry.id], heartbeat_at_ms: current } } }) : next); };
    const result = await execute(plan, { maxOutputBytes: this.config.runtime.max_output_bytes, onStart: (pid) => updateRuntime(this.config.runtime.root, runtime_id, (next) => ({ ...next, providers: { ...next.providers, [entry.id]: { provider: entry.id, tier: entry.tier, status: "running", pid, started_at_ms: Date.now(), heartbeat_at_ms: Date.now(), session_id: prior?.session_id ?? null } } })), onActivity: heartbeat });
    const cancelled = cancellationRequested(this.config.runtime.root, runtime_id, entry.id);
    if (cancelled) return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, error: { code: "CANCELLED", message: "cancel was requested by the caller" } });
  ...
  #store(runtime_id, entry, result) { updateRuntime(this.config.runtime.root, runtime_id, (next) => ({ ...next, providers: { ...next.providers, [entry.id]: { provider: entry.id, tier: entry.tier, ...result, completed_at_ms: Date.now() } } })); return { provider: entry.id, tier: entry.tier, ...result }; }
  #finish(runtime_id, input, providers) { const state = updateRuntime(this.config.runtime.root, runtime_id, (next) => ({ ...next, round: next.round + 1, last_prompt_bytes: Buffer.byteLength(input.prompt, "utf8"), last_completed_at_ms: Date.now() })); return { version: 4, runtime_id, round: state.round, host_provider: state.host_provider, providers }; }
}
