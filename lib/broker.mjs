...
import { enforcePromptBudget, planDelivery, prepareCheckedAttachments, prepareWritableAttachmentView, probeAttachmentWorkspace, renderProviderPrompt, validateAttachmentRoot, validateAttachments, validateContinuationTriad, validateFileOnlyTriad, verifyFrozenAttachments, verifyWritableAttachmentView } from "./attachments.mjs";
  ...
  async run(raw) {
    ...
    if (continuing) {
      ...
      if (state.attachments && input.attachments) {
        ...
        continuationMaterial = { sequence: binding.sequence, bundle_id: checked.bundle_id, manifest_hash: checked.manifest_hash, delivery_manifest_hash: binding.delivery_manifest_hash, initial_material_manifest_hash: state.attachments.manifest_hash,
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial, rawDeliveryChecked, deliveryChecked) {
    ...
    try {
      ...
      if (input.attachments) {
        ...
        const frozen = verifyFrozenAttachments(runtime, entry.id, state.attachments.provider_material); deliveryUsed = prior?.delivery_used; delivery = prior?.delivery ?? null;
        ...
        if (!delivery || delivery.delivery_mode !== deliveryUsed || delivery.material_manifest_hash !== state.attachments.provider_material.manifest_hash || !Array.isArray(delivery.provider_visible_attachment_manifest)) fail("MATERIAL_INCOMPLETE", "continuation delivery record does not match frozen material");
    ...
    try {
      ...
      const providerCwd = worker.requiresWritableCwd ? cwd : (bundle ?? cwd);
      ...
      if (bundle && worker.requiresWritableCwd) verifyWritableAttachmentView(runtime, attachmentKey, attachmentStored ?? state.attachments.provider_material);
