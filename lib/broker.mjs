...
import { isSupportedProviderId } from "./provider-ids.mjs";
...
function request(value) {
  if (!value || value.version !== 4 || typeof value.prompt !== "string" || value.prompt.length === 0 || !isSupportedProviderId(value.host_provider)) fail("REQUEST_INVALID", "request needs version:4, a non-empty prompt, and a supported host_provider");
  ...
  if (value.provider_allowlist !== undefined) {
    if (!Array.isArray(value.provider_allowlist) || value.provider_allowlist.length === 0 || value.provider_allowlist.some((provider) => !isSupportedProviderId(provider) || provider === value.host_provider) || new Set(value.provider_allowlist).size !== value.provider_allowlist.length) fail("REQUEST_INVALID", "provider_allowlist must contain unique supported heterologous providers");
  }
