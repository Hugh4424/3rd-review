...
function managedPublic(operation, runtime_id) {
  const value = { version: "workflowhub-run.v1", request_id: operation.request_id, runtime_id, state: operation.state, material_id: operation.material_id };
  return operation.state === "terminal" ? { ...value, group: operation.group } : value;
}
function assertManagedPublic(value) {
  ...
  if (value.state === "terminal") assertWorkflowHubV2Group(value.group);
...
function assertWorkflowHubV2Group(value) {
  if (!value || typeof value !== "object" || value.version !== 4 || typeof value.runtime_id !== "string" || !Array.isArray(value.providers) || !value.providers.every((item) => item?.result_protocol === WORKFLOWHUB_RESULT_PROTOCOL_V2)) fail("PUBLIC_RESULT_INVALID", "managed terminal result is not a workflowhub-result.v2 group");
  value.providers.forEach(assertWorkflowHubV2ProviderSchema); assertNoPrivatePaths(value, "managed terminal group"); return value;
}
function managedTerminalGroup(value) {
  ...
  return assertWorkflowHubV2Group(group);
}
...
function managedBinding(input, checked, config) {
  const requestValue = structuredClone(input); requestValue.attachments = { delivery: checked.requested_delivery, manifest: input.attachments.manifest };
...
function publicV2Error(value) {
  if (!value) return null;
  const code = typeof value.code === "string" && value.code.length > 0 ? value.code : "UNKNOWN";
  ...
  return message && containsPrivatePath(message) ? { code, message: "provider error message omitted because it contained a private absolute path" } : { code, message };
}
function assertWorkflowHubV2ProviderSchema(value) {
  ...
  if (value.unavailable_diagnostics !== null) exactKeys(value.unavailable_diagnostics, workflowHubV2DiagnosticFields, "provider unavailable diagnostics");
  if (value.error !== null) exactKeys(value.error, workflowHubV2DiagnosticFields, "provider error");
...
function workflowHubResultV2(value, checked, profile, runtime_id, persisted = null) {
  ...
  const error = publicV2Error(value.error ?? facts.error ?? null);
  const result = {
    provider,
    ...
    unavailable_diagnostics: status === "completed" ? null : error ? { code: error.code ?? "UNKNOWN", message: error.message ?? null } : { code: "RESULT_UNAVAILABLE", message: null },
    output: value.output ?? null,
    error,
  };
...
export class Broker {
  ...
  managedStatus(runtime_id, operation_id = null) {
    ...
    const snapshotPath = managedPublicPath(this.config.runtime.root, runtime_id);
    if (fs.existsSync(snapshotPath)) {
      const snapshot = assertManagedPublic(readPrivateJson(snapshotPath, "RUNTIME_INVALID", "managed public status is unavailable"));
      if (snapshot.request_id === operation.request_id) return snapshot;
    }
  ...
  async runManagedOperation(runtime_id, operation_id, job) {
    ...
    group = managedTerminalGroup(group);
