...
function sumUsage(left, right) {
  ...
  if (!isDeepStrictEqual(leftKeys, rightKeys) || leftKeys.some((key) => !Number.isSafeInteger(left[key]) || left[key] < 0 || !Number.isSafeInteger(right[key]) || right[key] < 0)) return null;
  return Object.fromEntries(leftKeys.map((key) => [key, left[key] + right[key]]));
}
...
export class Broker {
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial, deliveryChecked) {
    ...
    let result = await execute({ ...plan, onOutput: rawCapture.write, onHealthDiagnostic: (diagnostic) => touch({ last_health_diagnostic: diagnostic }) }, {
      ...this.processOptions,
    ...
    let parsed = parseOutput(captured?.stdout ?? result.stdout, captured?.stderr ?? result.stderr);
    ...
    // The raw transcript remains in the broker-private raw store. Pi is the
    // only adapter with an explicit, same-session public-output rewrite
    ...
    if (containsPrivatePath(parsed.text) && provider.adapter === "pi" && typeof worker.publicOutputRewritePrompt === "string" && worker.publicOutputRewritePrompt.length > 0) {
      let rewritePlan;
      try {
        rewritePlan = worker.resume(provider, providerCwd, parsed.session_id, worker.publicOutputRewritePrompt, runtime);
      } catch {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, error: { code: "PROVIDER_OUTPUT_REWRITE_SETUP_FAILED", message: "broker could not prepare a provider output rewrite" } });
      }
      if (cancellationRequested(this.config.runtime.root, runtime_id, entry.id)) {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
      ...
      const firstStartedAt = readRuntime(this.config.runtime.root, runtime_id).providers?.[entry.id]?.started_at_ms ?? Date.now() - result.duration_ms;
      const rewrite = await execute({ ...rewritePlan, onOutput: rewriteCapture.write, onHealthDiagnostic: (diagnostic) => touch({ last_health_diagnostic: diagnostic }) }, {
        ...this.processOptions,
        maxOutputBytes: this.config.runtime.max_output_bytes,
        acceptSemanticOutputAfterStdinClose: true,
        livenessIntervalMs: this.config.runtime.liveness_interval_ms,
        isCancelled: () => cancellationRequested(this.config.runtime.root, runtime_id, entry.id),
        validateCompleted: (raw) => (rewritePlan.parseLive?.() ?? worker.parse(raw.stdout, raw.stderr, rewritePlan.expectedSession)).ok,
        onStart: (pid) => {
          ...
          updateRuntime(this.config.runtime.root, runtime_id, (next) => ({ ...next, owner: { ...currentOwnerIdentity(), started_at_ms: current }, providers: { ...next.providers, [entry.id]: { ...next.providers[entry.id], provider: entry.id, tier: entry.tier, status: "running", pid, worker: processIdentity(pid), started_at_ms: next.providers?.[entry.id]?.started_at_ms ?? firstStartedAt, process_alive_at_ms: current, last_progress_at_ms: null, session_id: parsed.session_id, ...(deliveryUsed ? { delivery_used: deliveryUsed } : {}), ...(delivery ? { delivery } : {}) } } }));
          if (!this.processOptions.managedSession) ensureRuntimeGuardian(this.config.runtime.root, runtime_id);
      ...
      if (rewriteCaptureError) {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        return this.#store(runtime_id, entry, { status: "failed", duration_ms: mergedDuration, ...telemetry, ...rawRefs, ...deliveryOutcome, error: { code: "RAW_OUTPUT_WRITE_FAILED", message: "broker could not persist provider raw output" } }, privateRewrite);
      }
      if (cancellationRequested(this.config.runtime.root, runtime_id, entry.id)) {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        const source = cancellationSource(this.config.runtime.root, runtime_id, entry.id); const invalidSource = source === INVALID_CANCELLATION_SOURCE;
        return this.#store(runtime_id, entry, { status: "cancelled", duration_ms: mergedDuration, ...telemetry, ...rawRefs, ...deliveryOutcome, cancellation_source: source, error: invalidSource ? { code: "CANCEL_SOURCE_INVALID", message: "cancel marker source is invalid", source } : { code: "CANCELLED", message: `cancel was requested by ${source}`, source } }, privateRewrite);
      }
      if (!rewrite.ok) {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        return this.#store(runtime_id, entry, { status: "failed", duration_ms: mergedDuration, ...telemetry, ...rawRefs, ...deliveryOutcome, error: rewrite.error, diagnostic: rewrite.stdout_truncated || rewrite.stderr_truncated ? "provider output summary truncated; inspect private raw output" : short(`${rewrite.stdout}\n${rewrite.stderr}`) }, privateRewrite);
      }
      const rewriteCaptured = rewrite.stdout_truncated || rewrite.stderr_truncated ? rewriteCapture.read() : null;
      if ((rewrite.stdout_truncated || rewrite.stderr_truncated) && !rewriteCaptured) {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        return this.#store(runtime_id, entry, { status: "failed", duration_ms: mergedDuration, ...telemetry, ...rawRefs, ...deliveryOutcome, error: { code: "RAW_OUTPUT_READ_FAILED", message: "broker could not read provider raw output for parsing" } }, privateRewrite);
      }
      const rewritten = rewritePlan.parseLive?.() ?? worker.parse(rewriteCaptured?.stdout ?? rewrite.stdout, rewriteCaptured?.stderr ?? rewrite.stderr, rewritePlan.expectedSession);
      if (!rewritten.ok) {
        this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        return this.#store(runtime_id, entry, { status: "failed", duration_ms: mergedDuration, ...telemetry, ...rawRefs, ...deliveryOutcome, error: rewrite.stdin_error ?? rewritten.error, diagnostic: rewrite.stdout_truncated || rewrite.stderr_truncated ? "provider output summary truncated; inspect private raw output" : short(`${rewrite.stdout}\n${rewrite.stderr}`) }, privateRewrite);
      }
    ...
    // adapter, remains a failed provider. No raw text is sanitized or made
    // public merely to obtain a semantic review result.
    if (containsPrivatePath(parsed.text)) {
      this.#releaseContinuationReservation(runtime_id, continuationMaterial, entry.id, prior?.session_id);
      return this.#store(runtime_id, entry, { status: "failed", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, error: { code: "PUBLIC_RESULT_INVALID", message: "provider output omitted because it contained a private absolute path" } }, privateRewrite ?? undefined);
    }
    ...
    return this.#store(runtime_id, entry, { status: "completed", duration_ms: result.duration_ms, ...telemetry, ...rawRefs, ...deliveryOutcome, session_id: parsed.session_id, usage: parsed.usage, output: parsed.text }, { timeout_retry: undefined, ...(privateRewrite ?? {}) });
  }
