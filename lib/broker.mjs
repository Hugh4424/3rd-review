...
import { deriveProviderMaterial, prepareSensitiveRoots, restoreSensitiveRoots, sensitiveRootSetHash } from "./material-redaction.mjs";
...
const timeoutMaterialFields = ["sequence", "bundle_id", "manifest_hash", "delivery_manifest_hash", "initial_material_manifest_hash", "provider_material_manifest_hash", "provider_delivery_manifest_hash", "provider_initial_material_manifest_hash", "redaction_root_set_hash"];
...
function legacyTimeoutCandidate(item) {
  return item?.status === "failed" && item.error?.code === "PROCESS_TIMEOUT" && hasSession(item) && item.timeout_retry === undefined
    && !item.cancellation_source && !item.error?.source && noSemanticResult(item) && typeof item.delivery_used === "string" && item.delivery && typeof item.delivery === "object";
}
...
function continuationEligible(item, material, candidateOnly = false) { return item?.status === "completed" || (item?.status === "running" && isAlive(item.pid)) || legacyTimeoutCandidate(item) || (candidateOnly ? timeoutRetryCandidate(item) : timeoutRetryEligible(item, material)); }
function validLegacyMaterial(material) {
  return Number.isSafeInteger(material?.sequence) && material.sequence > 0
    && timeoutMaterialFields.slice(1).every((field) => typeof material?.[field] === "string" && material[field].length > 0);
}
function validateLegacyDelivery(item, material, deliveryUsed, delivery) {
  const required = ["delivery_mode", "raw_material_manifest_hash", "material_manifest_hash", "material_representation", "redaction", "material_total_bytes", "provider_visible_attachment_manifest"];
  if (!validLegacyMaterial(material) || item.delivery_used !== deliveryUsed || required.some((field) => !Object.hasOwn(item.delivery, field)) || !isDeepStrictEqual(item.delivery, delivery)) fail("MATERIAL_INCOMPLETE", "legacy timeout delivery does not match the current unpublished material");
  if (delivery.redaction?.root_set_hash !== material.redaction_root_set_hash || delivery.redaction?.raw_material_manifest_hash !== material.manifest_hash || delivery.redaction?.derived_material_manifest_hash !== material.provider_material_manifest_hash) fail("MATERIAL_INCOMPLETE", "legacy timeout redaction binding is incomplete");
}
    ...
    if (fs.existsSync(dotGit)) {
      ...
      try {
        if (fs.lstatSync(dotGit).isFile()) {
          const gitdir = fs.readFileSync(dotGit, "utf8").match(/^gitdir:\s*(.+)\s*$/mu)?.[1];
          const resolved = gitdir ? path.resolve(current, gitdir) : null; const marker = `${path.sep}.git${path.sep}worktrees${path.sep}`;
          if (resolved?.includes(marker)) repo = resolved.slice(0, resolved.indexOf(marker));
        }
      } catch { /* lexical worktree root remains usable */ }
      return { worktree: current, repo };
    }
    const parent = path.dirname(current); if (parent === current) return { worktree: cwd, repo: cwd }; current = parent;
  }
}
function sensitiveRoots(config) {
  const cwd = process.cwd(); const pwd = process.env.PWD && path.isAbsolute(process.env.PWD) ? process.env.PWD : cwd; const git = gitRoots(cwd);
  return [
    { root_id: "worktree", values: [pwd, cwd, git.worktree] },
    { root_id: "repo", values: [git.repo] },
    { root_id: "temp", values: [tmpdir(), config.runtime.root] },
    { root_id: "home", values: [homedir()] },
    ...config.attachment_roots.map((item, index) => ({ root_id: `secret-${index + 1}`, values: [item.root] })),
  ];
}
...
export class Broker {
  constructor(config, options = {}) { const { beforeLegacyMigration, ...processOptions } = options; this.config = config; this.processOptions = processOptions; this.beforeLegacyMigration = beforeLegacyMigration; this.active = new Map(); this.shuttingDown = false; cleanup(config.runtime.root, config.runtime.ttl_hours); }
  ...
  async run(raw) {
    ...
    let materialRoots = [];
    if (continuing && state.attachments) {
      materialRoots = restoreSensitiveRoots(state.attachments.redaction_roots);
      const rootSetHash = sensitiveRootSetHash(materialRoots);
      if (state.attachments.redaction_root_set_hash !== rootSetHash || state.attachments.provider_material?.redaction?.root_set_hash !== rootSetHash) fail("MATERIAL_INCOMPLETE", "frozen sensitive host root set changed");
    } else if (!continuing) materialRoots = prepareSensitiveRoots(sensitiveRoots(this.config));
    ...
    let rawDeliveryChecked = initialChecked;
    let deliveryChecked = initialChecked ? deriveProviderMaterial(initialChecked, null, materialRoots) : null;
    ...
    if (continuing) {
      ...
      if (reuseFrozenMaterial && !state.attachments?.provider_material) fail("MATERIAL_INCOMPLETE", "reuse_frozen_material requires frozen initial provider material");
      ...
      if (state.attachments && input.attachments) {
        ...
        rawDeliveryChecked = checked;
        const previous = state.continuation_materials ?? [];
        if (!state.attachments.provider_material) fail("MATERIAL_INCOMPLETE", "continuation has no frozen provider material");
        deliveryChecked = deriveProviderMaterial(checked, { initial_material_manifest_hash: state.attachments.provider_material.manifest_hash, previous_delivery_manifest_hash: previous.length ? previous.at(-1).provider_delivery_manifest_hash : null, rule_version: state.attachments.provider_material.redaction?.rule_version, root_set_hash: state.attachments.redaction_root_set_hash }, materialRoots);
        continuationMaterial = { sequence: binding.sequence, workspace_round: state.round + 1, bundle_id: checked.bundle_id, manifest_hash: checked.manifest_hash, delivery_manifest_hash: binding.delivery_manifest_hash, initial_material_manifest_hash: state.attachments.manifest_hash,
          provider_material_manifest_hash: deliveryChecked.manifest_hash, provider_delivery_manifest_hash: deliveryChecked.delivery_manifest_hash, provider_initial_material_manifest_hash: state.attachments.provider_material.manifest_hash, redaction_root_set_hash: deliveryChecked.redaction?.root_set_hash };
      }
    } else if (input.attachments) {
      const checked = initialChecked;
      state = updateRuntime(this.config.runtime.root, state.runtime_id, (next) => ({ ...next, attachments: { requested_delivery: checked.requested_delivery, bundle_id: checked.bundle_id, manifest_hash: checked.manifest_hash, files: checked.files, redaction_roots: materialRoots, redaction_root_set_hash: sensitiveRootSetHash(materialRoots),
        provider_material: { bundle_id: deliveryChecked.bundle_id, manifest_hash: deliveryChecked.manifest_hash, files: deliveryChecked.files, material_representation: deliveryChecked.material_representation, redaction: deliveryChecked.redaction } } }));
    }
    ...
    if (continuing) output.push(...await Promise.all(entries.map((entry) => this.#runProvider(state.runtime_id, input, entry, continuationMaterial, rawDeliveryChecked, deliveryChecked))));
    else {
      for (const tier of entries) {
        const results = await Promise.all(tier.map((entry) => this.#runProvider(state.runtime_id, input, entry, null, rawDeliveryChecked, deliveryChecked))); output.push(...results);
  ...
  async #runProvider(runtime_id, input, entry, continuationMaterial, rawDeliveryChecked, deliveryChecked) {
    ...
    try { return await this.#runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial, rawDeliveryChecked, deliveryChecked); }
    finally { releaseProviderClaim(claim); }
  }
  async #runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial, rawDeliveryChecked, deliveryChecked) {
    ...
    try {
      ...
      if (input.attachments) {
        ...
        if (!rawDeliveryChecked || !deliveryChecked || deliveryChecked.raw_material_manifest_hash !== rawDeliveryChecked.manifest_hash) fail("MATERIAL_INCOMPLETE", "delivery material is not bound to the validated raw attachment snapshot");
        const rawChecked = rawDeliveryChecked; const checked = deliveryChecked;
        attachmentKey = entry.continuation ? `${entry.id}-delta-${state.round + 1}` : entry.id;
        const rawKey = `raw-${attachmentKey}`;
        if (fs.existsSync(path.join(runtime, "workspace", rawKey))) verifyFrozenAttachments(runtime, rawKey, rawChecked);
        else prepareCheckedAttachments(rawChecked, runtime, rawKey);
        deliveryUsed = input.attachments.delivery;
        delivery = { delivery_mode: deliveryUsed, raw_material_manifest_hash: rawChecked.manifest_hash, material_manifest_hash: checked.manifest_hash, material_representation: checked.material_representation, redaction: checked.redaction, material_total_bytes: checked.total_bytes, provider_visible_attachment_manifest: checked.files.map(({ target: destination, sha256, size }) => ({ destination, sha256, size })) };
        ...
        delivery = { delivery_mode: planned.delivery_mode, raw_material_manifest_hash: rawChecked.manifest_hash, material_manifest_hash: planned.material_manifest_hash, material_representation: checked.material_representation, redaction: checked.redaction, material_total_bytes: planned.material_total_bytes, ...(planned.rendered_prompt_bytes !== undefined ? { rendered_prompt_bytes: planned.rendered_prompt_bytes } : {}), provider_visible_attachment_manifest: planned.provider_visible_attachment_manifest };
        if (entry.continuation) {
          let reserved = false;
          verifyFrozenAttachments(runtime, entry.id, state.attachments.provider_material);
          if (timeoutRetryEligible(prior, continuationMaterial)) {
            if (prior.delivery?.delivery_mode !== delivery.delivery_mode || prior.delivery?.raw_material_manifest_hash !== delivery.raw_material_manifest_hash || prior.delivery?.material_manifest_hash !== delivery.material_manifest_hash) fail("MATERIAL_INCOMPLETE", "timeout retry delivery does not match its unpublished material");
          } else if (legacyTimeoutCandidate(prior)) {
            validateLegacyDelivery(prior, continuationMaterial, deliveryUsed, delivery);
            this.beforeLegacyMigration?.();
            this.#migrateLegacyTimeoutAndReserve(runtime_id, continuationMaterial, entry.id, prior.session_id, deliveryUsed, delivery); reserved = true;
          } else {
            const priorMaterial = lastProviderMaterial(state, entry.id, prior?.session_id);
            const expectedMaterialHash = priorMaterial?.provider_material_manifest_hash ?? state.attachments.provider_material.manifest_hash;
          ...
          if (!reserved) this.#reserveContinuationMaterial(runtime_id, continuationMaterial, entry.id, prior?.session_id);
        }
        const prepared = prepareCheckedAttachments(checked, runtime, attachmentKey); attachmentStored = prepared;
        ...
        deliveryUsed = prior?.delivery_used; delivery = prior?.delivery ?? null; let frozenKey = entry.id; let rawStored = state.attachments; let providerStored = state.attachments.provider_material;
        if (input.continuation?.reuse_frozen_material) {
          const latest = lastProviderMaterial(state, entry.id, prior?.session_id);
          if (latest) {
            if (delivery?.raw_material_manifest_hash !== latest.manifest_hash || delivery?.material_manifest_hash !== latest.provider_material_manifest_hash) fail("MATERIAL_INCOMPLETE", "continuation delivery record does not match the latest provider/session material");
            frozenKey = `${entry.id}-delta-${latest.workspace_round ?? latest.sequence + 1}`;
            rawStored = frozenDescriptor(runtime, `raw-${frozenKey}`, { bundle_id: latest.bundle_id, manifest_hash: latest.manifest_hash, delivery_manifest_hash: latest.delivery_manifest_hash }, deliveryUsed);
            providerStored = frozenDescriptor(runtime, frozenKey, { bundle_id: latest.bundle_id, manifest_hash: latest.provider_material_manifest_hash, delivery_manifest_hash: latest.provider_delivery_manifest_hash }, deliveryUsed, delivery.provider_visible_attachment_manifest);
          } else if (delivery?.raw_material_manifest_hash !== state.attachments.manifest_hash || delivery?.material_manifest_hash !== state.attachments.provider_material.manifest_hash) fail("MATERIAL_INCOMPLETE", "continuation delivery record does not match initial provider material");
          verifyFrozenAttachments(runtime, `raw-${frozenKey}`, rawStored);
        }
        ...
        if (!delivery || delivery.delivery_mode !== deliveryUsed || delivery.material_manifest_hash !== providerStored.manifest_hash || !Array.isArray(delivery.provider_visible_attachment_manifest)) fail("MATERIAL_INCOMPLETE", "continuation delivery record does not match frozen material");
      ...
      else if (worker.requiresWritableCwd && worker.stableContinuationCwd && state.attachments) cwd = refreshWritableAttachmentView(runtime, entry.id, attachmentKey, attachmentStored ?? state.attachments.provider_material).cwd;
      else if (worker.requiresWritableCwd && state.attachments) cwd = prepareWritableAttachmentView(runtime, entry.id, state.attachments.provider_material).cwd;
    ...
    try {
      ...
      const providerCwd = worker.requiresWritableCwd && worker.stableContinuationCwd ? (bundle ?? cwd) : worker.requiresWritableCwd ? cwd : (bundle ?? cwd);
      ...
      if (bundle && worker.requiresWritableCwd) verifyWritableAttachmentView(runtime, worker.stableContinuationCwd ? entry.id : attachmentKey, attachmentStored ?? state.attachments.provider_material);
      else if (bundle) verifyFrozenAttachments(runtime, attachmentKey, attachmentStored ?? state.attachments.provider_material);
    }
    catch (error) { return this.#setupFailure(runtime_id, entry, deliveryUsed, delivery, error, entry.continuation && prior?.status === "completed", continuationMaterial, prior?.session_id); }
  ...
  #migrateLegacyTimeoutAndReserve(runtime_id, material, provider, session_id, deliveryUsed, delivery) {
    updateRuntime(this.config.runtime.root, runtime_id, (next) => {
      const item = next.providers?.[provider]; if (!legacyTimeoutCandidate(item) || item.session_id !== session_id) fail("MATERIAL_INCOMPLETE", "legacy timeout candidate changed before migration"); validateLegacyDelivery(item, material, deliveryUsed, delivery);
      const completed = next.continuation_materials ?? [];
      if (next.continuation_reservation || completed.length !== material.sequence - 1 || completed.some((entry) => entry.sequence === material.sequence) || !providerHasContinuationPredecessor(next, provider, session_id, material.sequence)) fail("MATERIAL_INCOMPLETE", "legacy timeout delta is no longer the next unpublished continuation");
      const migrated = { ...next, providers: { ...next.providers, [provider]: { ...item, timeout_retry: { version: 1, material } } } };
      return reserveContinuationMaterial(migrated, material, provider, session_id);
    });
  }
