...
import { appendEmbedded, prepareAttachments, prepareWritableAttachmentView, validateAttachments, verifyFrozenAttachments } from "./attachments.mjs";
...
export class Broker {
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider) {
    ...
    } catch (error) { return this.#setupFailure(runtime_id, entry, deliveryUsed, error); }
    let plan;
  ...
  #setupFailure(runtime_id, entry, deliveryUsed, error) {
    const result = { status: "failed", ...(deliveryUsed ? { delivery_used: deliveryUsed } : {}), error: publicError(error) };
    return deliveryUsed ? this.#store(runtime_id, entry, result) : { provider: entry.id, tier: entry.tier, ...result };
  }
