...
import { cancellationRequested, cancellationSource, claimProvider, createRuntime, cleanup, currentOwnerIdentity, ensureRuntimeGuardian, INVALID_CANCELLATION_SOURCE, processIdentity, readRuntime, releaseProviderClaim, requestCancellation, runtimeDirectory, terminateProcessTree, updateRunningProvider, updateRuntime, workerIdentityMatches } from "./runtime.mjs";
...
import { canonicalDeliveryManifestHash, canonicalMaterialManifestHash, planDelivery, prepareCheckedAttachments, prepareWritableAttachmentView, probeAttachmentWorkspace, refreshWritableAttachmentView, renderProviderPrompt, validateAttachmentRoot, validateAttachments, validateContinuationTriad, validateFileOnlyTriad, verifyFrozenAttachments, verifyWritableAttachmentView } from "./attachments.mjs";
...
export class Broker {
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial, deliveryChecked) {
    ...
    const result = await execute({ ...plan, onOutput: rawCapture.write, onHealthDiagnostic: (diagnostic) => touch({ last_health_diagnostic: diagnostic }) }, {
      ...
      onStart: (pid) => { this.processOptions.onStart?.(pid); const current = Date.now(); this.active.set(`${runtime_id}:${entry.id}`, { runtime_id, provider: entry.id, pid }); updateRuntime(this.config.runtime.root, runtime_id, (next) => ({ ...next, owner: { ...currentOwnerIdentity(), started_at_ms: current }, providers: { ...next.providers, [entry.id]: { provider: entry.id, tier: entry.tier, status: "running", pid, worker: processIdentity(pid), started_at_ms: current, process_alive_at_ms: current, last_progress_at_ms: null, session_id: prior?.session_id ?? null, ...(deliveryUsed ? { delivery_used: deliveryUsed } : {}), ...(delivery ? { delivery } : {}) } } })); ensureRuntimeGuardian(this.config.runtime.root, runtime_id); if (this.shuttingDown) { requestCancellation(this.config.runtime.root, runtime_id, entry.id, "workflow_shutdown"); terminateProcessTree(pid, "SIGTERM"); } },
