...
export class Broker {
  ...
  async #runClaimedProvider(runtime_id, input, entry, provider, continuationMaterial) {
    ...
    try {
      if (input.attachments) {
        ...
        if (entry.continuation) {
          ...
          if (prior?.delivery?.material_manifest_hash !== state.attachments.manifest_hash) fail("MATERIAL_INCOMPLETE", "continuation initial delivery record does not match its frozen material");
        }
    ...
    let plan;
    try { if (!worker.promptViaStdin) fs.writeFileSync(path.join(cwd, "review-input.md"), providerPrompt, { mode: 0o600 }); plan = entry.continuation ? worker.resume(provider, cwd, prior.session_id, providerPrompt, runtime) : worker.start(provider, cwd, providerPrompt, runtime); if (deliveryUsed === "file_only") { const bundle = fs.existsSync(path.join(cwd, "bundle")) ? path.join(cwd, "bundle") : cwd; plan = requireFileOnlySandbox(this.config, plan, bundle, cwd); } }
    catch (error) { return this.#setupFailure(runtime_id, entry, deliveryUsed, delivery, error, entry.continuation && prior?.status === "completed"); }
  ...
  #recordContinuationMaterial(runtime_id, material, provider, session_id) {
    ...
    updateRuntime(this.config.runtime.root, runtime_id, (next) => {
      const completed = next.continuation_materials ?? []; const existing = completed.find((item) => item.sequence === material.sequence);
      if (existing) {
        ...
        return { ...next, continuation_materials: completed.map((item) => item.sequence === material.sequence ? { ...item, provider_sessions: { ...item.provider_sessions, [provider]: session_id } } : item) };
      }
      if (completed.length !== material.sequence - 1) fail("MATERIAL_INCOMPLETE", "continuation sequence cannot be appended out of order");
      return { ...next, continuation_materials: [...completed, { ...material, provider_sessions: { [provider]: session_id } }] };
    });
  }
