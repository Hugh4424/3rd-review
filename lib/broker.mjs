...
export class Broker {
    ...
    try {
      if (input.attachments) {
        ...
        if (entry.continuation) {
          ...
          const priorMaterial = this.#lastProviderMaterial(state, entry.id, prior?.session_id);
  ...
  #recordContinuationMaterial(runtime_id, material, provider, session_id) {
    ...
    updateRuntime(this.config.runtime.root, runtime_id, (next) => {
      const completed = next.continuation_materials ?? []; const reservation = next.continuation_reservation;
      ...
      const existing = completed.find((item) => item.sequence === material.sequence);
      if (existing) {
        ...
        return { ...next, continuation_materials: completed.map((item) => item.sequence === material.sequence ? { ...item, provider_sessions: { ...item.provider_sessions, [provider]: session_id } } : item), ...(Object.keys(provider_sessions).length ? { continuation_reservation: { ...reservation, provider_sessions } } : { continuation_reservation: null }) };
      }
      if (completed.length !== material.sequence - 1) fail("MATERIAL_INCOMPLETE", "continuation sequence cannot be appended out of order");
      const provider_sessions = { ...reservation.provider_sessions }; delete provider_sessions[provider];
      return { ...next, continuation_materials: [...completed, { ...material, provider_sessions: { [provider]: session_id } }], ...(Object.keys(provider_sessions).length ? { continuation_reservation: { ...reservation, provider_sessions } } : { continuation_reservation: null }) };
    });
  }
  #lastProviderMaterial(state, provider, session_id) {
  ...
  #reserveContinuationMaterial(runtime_id, material, provider, session_id) {
    ...
    updateRuntime(this.config.runtime.root, runtime_id, (next) => {
      ...
      return { ...next, continuation_reservation: { ...(reservation ?? material), provider_sessions: { ...(reservation?.provider_sessions ?? {}), [provider]: session_id } } };
    });
  }
  #releaseContinuationReservation(runtime_id, material, provider, session_id) {
    if (!material || typeof session_id !== "string" || !session_id) return;
    updateRuntime(this.config.runtime.root, runtime_id, (next) => {
      const reservation = next.continuation_reservation;
      if (!reservation || reservation.sequence !== material.sequence || reservation.manifest_hash !== material.manifest_hash || reservation.provider_sessions?.[provider] !== session_id) return next;
      const provider_sessions = { ...reservation.provider_sessions }; delete provider_sessions[provider];
      return { ...next, continuation_reservation: Object.keys(provider_sessions).length ? { ...reservation, provider_sessions } : null };
    });
  }
