...
const trustedFileOnlySandboxes = new WeakSet();
...
function fileOnlySandbox(value) {
  ...
  const provider_visible_root = string(source.provider_visible_root, "config.file_only_sandbox.provider_visible_root");
  if (!path.posix.isAbsolute(provider_visible_root) || provider_visible_root.includes("..") || provider_visible_root === "/") fail("CONFIG_INVALID", "config.file_only_sandbox.provider_visible_root must be a safe virtual absolute path");
  ...
  return { command, args, sha256: sha256.toLowerCase(), provider_visible_root };
}
...
export function loadConfig(file = defaultConfigPath()) {
  try {
    ...
    if (config.file_only_sandbox) {
      const hostPath = path.resolve(defaultConfigPath()); const hostStat = fs.lstatSync(hostPath); const stat = fs.lstatSync(requested); const owned = stat.uid === process.getuid() || stat.uid === 0;
      if (requested === hostPath && hostStat.isFile() && !hostStat.isSymbolicLink() && stat.isFile() && !stat.isSymbolicLink() && owned && (stat.mode & 0o022) === 0) trustedFileOnlySandboxes.add(config.file_only_sandbox);
    }
    return config;
  }
...
export function hasTrustedFileOnlySandbox(value) { return Boolean(value && trustedFileOnlySandboxes.has(value)); }
