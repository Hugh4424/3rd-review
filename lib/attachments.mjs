...
export function canonicalDeliveryManifestHash(bundleId, files) { return digest(canonicalJson({ version: 1, bundle_id: bundleId, files: files.filter((item) => item.target !== "manifest.json").map(({ target, sha256, size }) => ({ target, sha256, size })) })); }
...
export function validateFileOnlyTriad(checked) {
  ...
  if (digestField(inner, "delivery_manifest_hash", "manifest.json") !== canonicalDeliveryManifestHash(checked.bundle_id, checked.files)) fail("MATERIAL_INCOMPLETE", "manifest.json delivery_manifest_hash does not bind the outer delivery manifest");
...
export function validateContinuationTriad(checked, initial) {
  ...
  if (typeof initial?.manifest_hash !== "string" || !/^[a-f0-9]{64}$/i.test(initial.manifest_hash)) fail("MATERIAL_INCOMPLETE", "continuation has no trusted initial material hash");
  ...
  if (!binding || typeof binding !== "object" || binding.initial_material_manifest_hash !== initial.manifest_hash || !Number.isSafeInteger(binding.sequence) || binding.sequence < 1) fail("MATERIAL_INCOMPLETE", "continuation manifest does not bind the initial material");
