...
export function validateFileOnlyTriad(checked) {
  ...
  if (checked.requested_delivery === "file_only" && inner.delivery_mode !== "file_only") fail("MATERIAL_INCOMPLETE", "manifest.json delivery_mode does not permit file_only delivery");
  ...
  if (digestField(inner, "delivery_manifest_hash", "manifest.json") !== canonicalDeliveryManifestHash(checked.bundle_id, checked.files, inner.delivery_mode)) fail("MATERIAL_INCOMPLETE", "manifest.json delivery_manifest_hash does not bind the outer delivery manifest");
