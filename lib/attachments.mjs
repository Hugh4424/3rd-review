...
export function verifyFrozenAttachments(runtime, provider, stored) {
  if (!stored) fail("ATTACHMENT_IMMUTABLE", "runtime has no frozen attachments"); const cwd = path.join(runtime, "workspace", provider); const manifest = path.join(cwd, "attachments-manifest.json"); const embedded = [];
  try {
  ...
  return { cwd, embedded_text: stored.files.every((item) => item.embed) ? renderEmbedded(embedded) : null };
}
