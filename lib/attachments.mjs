...
export function canonicalWorkflowHubMaterialId(files) {
  const entries = files.filter((item) => item.target !== "manifest.json")
...
export function planDelivery(worker, checked, prompt, maxPromptBytes, { requireTriad = true } = {}) {
  if (requireTriad) validateFileOnlyTriad(checked); else validateDeliveryFlags(checked);
