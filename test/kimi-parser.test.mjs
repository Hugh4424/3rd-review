...
test("Kimi receives only its generated private bundle path, never a host attachment root", () => {
  ...
  assert.equal(result.cwd, cwd); assert.equal(fs.statSync(cwd).mode & 0o200, 0o200); assert.equal(result.argv[result.argv.indexOf("--work-dir") + 1], cwd); assert.equal(result.argv[result.argv.indexOf("--skills-dir") + 1], skills); const system = fs.readFileSync(path.join(runtime, "kimi", "reviewer.md"), "utf8"); assert.match(system, /read and apply attached skills/i); assert.match(system, new RegExp(bundle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))); assert.match(system, /only permitted absolute file path/i); assert.match(system, /never output or request this path/i); assert.doesNotMatch(system, /relative `bundle` directory/i); assert.doesNotMatch(system, new RegExp(hostAttachmentRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))); assert.match(system, /review-packet\.v1\.json.*contracts.*skills/is);
});
