...
test("Kimi does not create a provider profile beside its attachment workspace", () => {
  ...
  const result = kimi.start({ id: "kimi", command: "kimi", model: null, thinking: null, auth: { env: [] }, env: [] }, bundle, "review", runtime);
  assert.equal(result.cwd, bundle); assert.equal(fs.statSync(cwd).mode & 0o200, 0o200); assert.equal(result.argv[result.argv.indexOf("--work-dir") + 1], bundle); assert.equal(result.argv[result.argv.indexOf("--skills-dir") + 1], skills); assert.equal(result.argv.includes("--agent-file"), false); assert.equal(fs.existsSync(path.join(cwd, ".broker-profile")), false); assert.doesNotMatch(result.argv.join(" "), new RegExp(hostAttachmentRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
});
