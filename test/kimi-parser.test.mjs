...
test("Kimi profile never places a host attachment path in its prompt", () => {
  ...
  assert.equal(result.cwd, cwd); assert.equal(fs.statSync(cwd).mode & 0o200, 0o200); assert.equal(result.argv[result.argv.indexOf("--work-dir") + 1], cwd); assert.equal(result.argv[result.argv.indexOf("--skills-dir") + 1], skills); const system = fs.readFileSync(path.join(cwd, ".broker-profile", "reviewer.md"), "utf8"); assert.match(system, /read and apply attached skills/i); assert.doesNotMatch(system, new RegExp(bundle.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))); assert.doesNotMatch(system, new RegExp(hostAttachmentRoot.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))); assert.match(system, /review-packet\.v1\.json.*contracts.*skills/is);
});
