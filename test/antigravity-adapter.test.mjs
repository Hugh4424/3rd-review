...
const fake = path.resolve("test/fake-antigravity-cli.mjs");
...
test("Antigravity rejects unsupported generic effort and unsafe argv prompts", () => {
  ...
  assert.throws(() => antigravity.start(provider(), temp(), "x".repeat(64 * 1024 + 1)), { code: "PROMPT_TOO_LARGE" });
});
