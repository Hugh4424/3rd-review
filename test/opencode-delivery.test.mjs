...
test("OpenCode receives the complete 80KB provider prompt on stdin", () => {
  ...
  assert.equal(result.input, prompt); assert.ok(Buffer.byteLength(result.input) > 80_000); assert.match(result.input, /^ATTACHMENT_HEAD/); assert.match(result.input, /ATTACHMENT_TAIL$/); assert.equal(result.argv.includes("--file"), false); assert.doesNotMatch(result.argv.join(" "), /review-input|Read tool|offset|EOF/i); assert.equal(result.argv.join(" ").includes(process.cwd()), false); assert.equal(fs.existsSync(path.join(cwd, "review-input.md")), false);
});
