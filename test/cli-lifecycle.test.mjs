...
test("concurrent continuation atomically claims a provider and preserves unique rounds", async () => {
  ...
  const firstCall = collect(process.execPath, [cli, "run", `--config=${config}`, `--request=${nextRequest}`]); const { runtime: runningRuntime } = await waitForRuntime(root); assert.equal(runningRuntime, initial.runtime_id); const calls = await Promise.all([firstCall, collect(process.execPath, [cli, "run", `--config=${config}`, `--request=${nextRequest}`])]); assert.deepEqual(calls.map((item) => item.code).sort(), [0, 3]); const results = calls.map((item) => JSON.parse(item.stdout));
