...
1. `verify-code-reviewer-contract.md` — acceptance review dimensions, blocking rules, historical pitfalls.
...
The first round of phase >= 2 must check the latest report from the previous phase. Recurring issues must be marked with `cross_phase_recurrence: true`; whether they block is determined by rule FR-REV-001 in `verify-code-reviewer-contract.md`.
