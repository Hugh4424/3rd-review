...
1. `build-plan-reviewer-contract.md` — plan review dimensions, blocking rules, historical pitfalls.
...
For phase >= 2, the first review round must check the most recent report from the previous phase. Mark recurring issues with `cross_phase_recurrence: true`; whether they are blocking is determined by the FR-REV-001 rule in `build-plan-reviewer-contract.md`.
